...
module.exports = async function () {
    ...
    bot.hears('✅ Оплатил', async (ctx) => {
        try {
            ...
            const waitOrderText = `🫠 <b>Ожидание оплаты</b>

⚠️ <u>Бот проверяет оплату, только за последние 3 дня.</u>

❗️Если этот срок истёк, то для повторной авторизации требуется снова провести транзакцию для подключения кошелька.`;
            ...
            if(getAllTransactions){
                ...
                if(getSpecificTransaction.status){
                    ...
                    ctx.reply(`<b>✅ Оплата принята.</b>

🤖 Осуществляем проверку нужных NFT...`, {parse_mode: 'Html'});
